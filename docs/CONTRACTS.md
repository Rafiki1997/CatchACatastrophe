...
| `client/Controllers/Waypoints.luau` | `init()`, `count()`, `titleHeights()` — one BillboardGui in `PlayerGui` over every `"RegionGate"`, every `"Waypoint"` tagged part (`WaypointName` / `WaypointColor` / `WaypointHeight`) and the player's own plot ("Your City"): the name in the destination's colour, `Theme.fonts.title`, ink `UIStroke` (Contextual), sized in pixels every frame (`RenderStepped`) as 44 x 7.6 studs (title 5, detail 2.6) would be at the camera depth, depth clamped to 35..200 studs, so it scales with distance but never balloons or vanishes (`DistanceLowerLimit` / `DistanceUpperLimit` do nothing, see RELAY traps); `AlwaysOnTop`, `MaxDistance` 900, hidden within 28 studs of the character. A locked gate adds a gold "Unlock X Coins" line; nothing else carries a second line. The outline thickness follows the letter height each frame, 7%, 1.5-4.5 px. | Play |
