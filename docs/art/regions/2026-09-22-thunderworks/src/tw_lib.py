# Shared helpers for the region mockup scenes: colour, materials, bmesh
# primitives and the scenery kit. Copied unchanged from the Frostbite Peaks
# round (../../2026-09-22-frostbite-peaks/src/fb_lib.py) so both rounds
# render alike; the Thunderworks additions live in tw_geo.py and tw_kit.py.
#
# Scene axes are region-local studs: +X east (the player's right walking in),
# +Y north, +Z up, the cell floor top at z = 0. Roblox (x, y, z) in the region
# frame is Blender (x, -z, y).
import bpy
import bmesh
import math
import random
